...
import * as Painter from './lib/painter';

const downloader = new Downloader();
const MAX_PAINT_COUNT = 5;

export interface PainterCanvasView {
...
interface PainterCanvasPalette {
    background?: string; // 整个模版的背景，支持网络图片的链接、纯色和渐变色
    width: string;
    height: string;
    borderRadius?: string;
    views: Array<PainterCanvasView>;
}
...
interface PainterCanvasProps {
    ...
    palette?: PainterCanvasPalette;
    dancePalette?: PainterCanvasPalette;
    ...
    dirty?: boolean;
    LRU?: boolean;
    action?: { view: PainterCanvasView };
    disableAction?: boolean;
    clearActionBox?: boolean;
    use2D?: boolean;
    onImgOK?: (path: string) => void;
    onImgErr?: (error: any) => void;
    onViewUpdate?: (view: PainterCanvasView | undefined) => void;
    onViewClicked?: (view: PainterCanvasView | undefined) => void;
    onTouchEnd?: (detail: { view?: PainterCanvasView | undefined; type?: string; index?: number }) => void;
    onDidShow?: () => void;
}

interface PainterCanvasState {
    photoStyle: string;
    painterStyle: string;
    picURL: string;
    showCanvas: boolean;
...
interface PainterCanvasRefs {
    paintCount: number;
    outterDisabled: boolean;
    isDisabled: boolean;
    imgSize: any;
    needClear: boolean;
    touchedView?: PainterCanvasView | any;
    prevFindedIndex?: number;
    findedIndex: number;
    oldPalette?: PainterCanvasPalette;
    currentPalette: PainterCanvasPalette;
    block?: PainterCanvasPalette;
    startX: number;
    startY: number;
    startH: number;
    startW: number;
...
const PainterCanvas: React.FC<PainterCanvasProps> = (props: PainterCanvasProps = {
    scaleRatio: 1,
    widthPixels: 0,
    dirty: false,
    LRU: true,
}) => {
    const [state, setData] = useState<Partial<PainterCanvasState>>({
        picURL: '',
        showCanvas: true,
        painterStyle: '',
    });
    const refs = useRef<PainterCanvasRefs>({
        imgSize: {},
        needClear: false,
        isDisabled: false,
        currentPalette: {
    ...
    function setState(params: Partial<PainterCanvasState>) {
        setData(pre => {
            return {
                ...pre,
                ...params,
    ...
    function doAction(action?: { view: PainterCanvasView } | null, overwrite?: boolean) {
        if (props.use2D) {
            return;
        }
        const newVal: PainterCanvasView | undefined = action ? action.view : undefined;
    ...
    function reDraw(doView: PainterCanvasView) {
        const draw: any = {
    ...
    function getCanvasContext(use2D, id): Promise<Taro.CanvasContext | WxCanvas> {
        return new Promise(resolve => {
            if (use2D) {
                ...
                const temp = Taro.createCanvasContext(id);
    ...
    return (
        <View style={`position: relative;${props.customStyle};${state.painterStyle};`}>
            {props.use2D ? (
                ...
                <Block>
                    <Canvas canvas-id="photo"
                            style={`${state.photoStyle};position: absolute; left: -9999px; top: -9999rpx;`}/>
